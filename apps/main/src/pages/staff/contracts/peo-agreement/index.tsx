...
import {
  StaffContractsPeoAgreementFormValues,
  validationSchema,
} from '@components/pages/staff/contracts/peo-agreement/config';
        ...
        >
          <Form>
            <Autocomplete
              ...
              dataTestId="staffContractsPeoAgreement-sectionNameSelectField"
            />
            <Box
              sx={{
                gap: '2rem',
...
const StaffContractsPeoAgreement: NextPage = ({ session, isDesktop }) => {
  const [loading, setLoading] = useState(true);
  const [toast, setToast] = useState<Toast>({});
  const { t } = useTranslation('staff-contracts-peo-agreement');
  ...
  const [initialValues, setInitialValues] =
    useState<StaffContractsPeoAgreementFormValues>({
      content: '',
    });
        ...
        if (country) {
          ...
          const { agreementTemplate, autoFillTags } =
            await AgreementTemplateApi.get(
              session,
              `${PEO_CONTRACT_AGREEMENT_TEMPLATE_PREFIX}${country?.code.toLowerCase()}`
            );
  ...
  const form = useMemo(() => {
    const onSubmit = async (values: StaffContractsPeoAgreementFormValues) => {
      try {
        await AgreementTemplateApi.update(session, {
          agreementTemplateId: `${PEO_CONTRACT_AGREEMENT_TEMPLATE_PREFIX}${country?.code.toLowerCase()}`,
        ...
        <Form noValidate>
          ...
          >
            <ButtonSubmit
              ...
              data-testid="staffContractsPeoAgreement-submitButton"
            >
              {t('common:submit')}
            </ButtonSubmit>
          </Box>
  ...
  return (
    ...
    >
      ...
      <Toast
        ...
        dataTestId="staffContractsPeoAgreement-toast"
      >
        {toast.message}
      </Toast>
      <Box
...
export default StaffContractsPeoAgreement;
